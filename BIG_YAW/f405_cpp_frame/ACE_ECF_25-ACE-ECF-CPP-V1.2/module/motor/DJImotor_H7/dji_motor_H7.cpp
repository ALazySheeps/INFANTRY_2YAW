        ...
        for (size_t i = 0; i < DJI_Motor_Instance::dji_motor_idx; ++i)
        {
            ...
            if(motor->stop_flag != Motor_General_Def_n::MOTOR_OUTPUT_ONLY_ME)
            {
                ...
                switch (motor->motor_type)
                {
                    case Motor_General_Def_n::GM6020:
                    {
                        if(motor->set > motor->Current_limit_H_6020)
                            motor->set = (int16_t)motor->Current_limit_H_6020;
                        else if(motor->set < motor->Current_limit_L_6020)
                            motor->set = (int16_t)motor->Current_limit_L_6020;      
                        break;
                    }
                    case Motor_General_Def_n::M3508:
                    {
                        if(motor->set > motor->Current_limit_H_3508)
                            motor->set = (int16_t)motor->Current_limit_H_3508;
                        else if(motor->set < motor->Current_limit_L_3508)
                            motor->set = (int16_t)motor->Current_limit_L_3508;      
                        break;
                    }
                    case Motor_General_Def_n::M2006:
                    {
