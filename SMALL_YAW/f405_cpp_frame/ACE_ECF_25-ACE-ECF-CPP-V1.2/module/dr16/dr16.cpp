...
namespace DR16_n
{
    ...
    USART_N::usart_init_t dr16_uart_param =
    {
     ...
     Safe_task_c *dr16_lost_ = new Safe_task_c("dr16",500,&(DR16_n::LostHandle),nullptr); 
    
    /// @brief dr16的父类构造
    /// @param dead_line_ 
    DR16_c::DR16_c(int16_t dead_line_): dead_line_(dead_line_), lost_time_(0)
    {
        dr16_->RC_HandleData.rc.s1 = RC_SW_DOWN;
        dr16_->RC_HandleData.rc.s2 = RC_SW_DOWN;
    ...
    void DR16_c::RC_Data_init_Clear()
    {
        dr16_->RC_HandleData.rc.ch[0] = 0;
        dr16_->RC_HandleData.rc.ch[1] = 0;
        dr16_->RC_HandleData.rc.ch[2] = 0;
        dr16_->RC_HandleData.rc.ch[3] = 0;
        dr16_->RC_HandleData.rc.ch[4] = 0;

        dr16_->RC_HandleData.kb.key_code = 0;
    }
