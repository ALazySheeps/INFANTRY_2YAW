 ...
 * 2.为其赋值// rx_size,rx/tx_callback函数，接收发送发送等，具体看hpp,根据需求不做初始化也可以
...
using namespace USART_N;
    ...
    void usart_c::USART_rx_start()
    {
        ...
        case USART_RX_DMA_IDLE_D:
            USART_RxDMA_DoubleBuffer_Init(usart_handle, (uint32_t *)rx_buff_ptr, (uint32_t *)sec_rxbuff_ptr, rxbuf_size);
            memset(rx_buff_ptr, 0, rxbuf_size);    // 清空接收缓冲区
    ...
    void usart_c::USART_RxDMA_DoubleBuffer_Init(UART_HandleTypeDef * huart, uint32_t * DstAddress, uint32_t * SecondMemAddress, uint32_t DataLength)
    {
            ...
            if (temp_ptr->usart_handle == huart && temp_ptr->usart_rx_callback_ptr != nullptr)
            {
                if (temp_ptr->rx_type == USART_RX_DMA_IDLE_D) // 如果为双缓冲中断
                {
                    ...
                    if (((((DMA_Stream_TypeDef *)huart->hdmarx->Instance)->CR) & DMA_SxCR_CT) == RESET)
                    {
                        ...
                        __HAL_DMA_SET_COUNTER(huart->hdmarx, temp_ptr->rxbuf_size); // 设置数据长度

                        if (Size == temp_ptr->usart_data_length)
                        {
                    ...
                    else
                    {
                        ...
                        __HAL_DMA_SET_COUNTER(huart->hdmarx, temp_ptr->rxbuf_size);
