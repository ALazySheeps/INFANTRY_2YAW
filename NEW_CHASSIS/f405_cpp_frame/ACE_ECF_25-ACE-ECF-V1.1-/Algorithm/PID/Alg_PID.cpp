...
//无参构造函数
pid_alg_c::pid_alg_c():out_filter(0),d_filter(0)
{
...
float cascade_pid_c::Cascade_pid_Calculate(const float *SetPosVal)
{
    ...
    outer_pid_output = (Outer_ActualVal==nullptr) ? 
        outer_pid.ECF_PID_Calculate(*SetPosVal,0) : outer_pid.ECF_PID_Calculate(*SetPosVal,*Outer_ActualVal);
    //加入速度前馈
    if(SpeedFeedWard != nullptr)
        outer_pid_output += *SpeedFeedWard;
    //计算内环
    inter_pid_output = (Inter_ActualVal==nullptr) ? 
        inter_pid.ECF_PID_Calculate(outer_pid_output,0) : inter_pid.ECF_PID_Calculate(outer_pid_output,*Inter_ActualVal);
...
float cascade_pid_c::Cascade_pid_Calculate(const float *SetPosVal, const float *outer_ActValSource,const float *inter_ActValSource)
{
    ...
    outer_pid_output = (outer_ActValSource==nullptr) ? 
        outer_pid.ECF_PID_Calculate(*SetPosVal,0) : outer_pid.ECF_PID_Calculate(*SetPosVal,*outer_ActValSource);
    //加入速度前馈
    if(SpeedFeedWard != nullptr)
        outer_pid_output += *SpeedFeedWard;
    //计算内环
    inter_pid_output = (inter_ActValSource==nullptr) ? 
        inter_pid.ECF_PID_Calculate(outer_pid_output,0) : inter_pid.ECF_PID_Calculate(outer_pid_output,*inter_ActValSource);
    user_value_limit(inter_pid_output,-Output_Limit,Output_Limit);
    return inter_pid_output;
}
