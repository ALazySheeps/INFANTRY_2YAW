...
namespace DJI_Motor_n
{
    ...
    DJI_Motor_Instance::DJI_Motor_Instance(Motor_General_Def_n::Motor_Init_Config_s config) : motor_can_instance(config.can_init_config)
    {
        ...
        this->lost_detection_dji  = new Safe_task_c("Dji_motor",50,[this] () {DJI_Disconnect();},nullptr);
    ...
    void DJI_Motor_n::DJI_Motor_Instance::DJIMotorSelfLock(void)
    {
        ...
        this->motor_settings.close_loop_type = Motor_General_Def_n::ANGLE_LOOP;
        this->motor_settings.outer_loop_type = Motor_General_Def_n::ANGLE_LOOP;
    }
    /**
     * @brief Dji 电机解除自锁
    ...
    void DJI_Motor_Instance::DJIMotorClearSelfLock(void)
    {
        this->stop_flag = Motor_General_Def_n::MOTOR_ENALBED;
        this->motor_settings.outer_loop_type = Motor_General_Def_n::SPEED_LOOP;
        this->motor_settings.close_loop_type = Motor_General_Def_n::SPEED_LOOP;
    }
  ...
  */
    ...
    void DJIMotorControl()
    {
        ...
        for (size_t i = 0; i < DJI_Motor_Instance::dji_motor_idx; ++i)
        {
            ...
            // 若该电机所注册在的CAN总线只用于接收，则该电机不发送且别把电机注册在该总线上面的ID分组上面
                ...
                motor_setting->outer_loop_type == Motor_General_Def_n::ANGLE_LOOP)
            {
                ...
                {
                    // 默认用累计编码值做闭环
                    if (motor->motor_controller.angle_PID.ECF_PID_GetActValSource() == nullptr)
                    {
                        // 把累计编码器赋值给float的变量
                        //motor->MotorMeasure.measure.record_ecd_to_pid = motor->MotorMeasure.measure.record_ecd;
            ...
            {
                motor->MotorMeasure.measure.feedback_real_current_to_pid = motor->MotorMeasure.measure.feedback_real_current;
                motor->motor_controller.current_PID.ECF_PID_ChangeActValSource(&motor->MotorMeasure.measure.feedback_real_current_to_pid);
            ...
            case Motor_General_Def_n::M3508:
            {
                if (motor->set > motor->Current_limit_H_3508_6020)
                    motor->set = (int16_t)motor->Current_limit_H_3508_6020;
                else if (motor->set < motor->Current_limit_L_3508_6020)
                    motor->set = (int16_t)motor->Current_limit_L_3508_6020;
                break;
            }
            case Motor_General_Def_n::M2006:
            {

                if (motor->set > motor->Current_limit_H_2006)
                    motor->set = (int16_t)motor->Current_limit_H_2006;
                else if (motor->set < motor->Current_limit_L_2006)
                    motor->set = (int16_t)motor->Current_limit_L_2006;
                break;
            }
    ...
    // 修改位置环的反馈为电机传回的其他变量值
    void DJI_Motor_Instance::Set_ANGLE_PID_Motor_feedback(float *ptr)
    {
        this->motor_settings.angle_feedback_source = Motor_General_Def_n::MOTOR_FEED;
        this->other_angle_feedback_ptr_ = ptr;
    }
    // 修改位置环为其他反馈指针
    ...
    // 修改3508、6020的自定义限幅
    void DJI_Motor_Instance::Set_Current_limit_H_3508_6020(int16_t high)
    {
        this->Current_limit_H_3508_6020 = high;
    }
    void DJI_Motor_Instance::Set_Current_limit_L_3508_6020(int16_t low)
    {
        this->Current_limit_L_3508_6020 = low;
    }
    // 修改2006电机的自定义限幅
    ...
    {
        return this->MotorMeasure.measure;
    }
    ...
    // 获取电机的最终给定电流
    int16_t DJI_Motor_Instance::Get_Motor_Give_current()
    {
        return this->give_current;
    }
    /***
    ...
    void DJI_Motor_Instance::DJI_Disconnect()
    {
        this->is_deline = true;
        this->DJIMotorStop();
    }

    bool DJI_Motor_Instance::Get_Lost_Status()
    {
        return this->is_deline;
    }

    void DJI_Motor_Instance::DJI_Online()
    {
        lost_detection_dji->Online();
        is_deline = false;
    }
}
